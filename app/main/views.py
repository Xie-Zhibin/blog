...
def postarticle():
    ...
    if (data):
        ...
        if (check):
            ...
            picKey = data.get("key")
            if (title and summary1 and artType and picKey):
                newArt = Articles(title=title, summary1=summary1,
                                  summary2=summary2, artType=artType,
                                  picKey=picKey)
...
def sum():
    ...
    raw_data = {"data":{"all":all_num,"coding":coding_num,"others":others_num}, "status":1}
    json_data = json.dumps(raw_data)
    return json_data
...
# def notFound(e):
#     return jsonify(errmsg="somethingwrong",status=0)